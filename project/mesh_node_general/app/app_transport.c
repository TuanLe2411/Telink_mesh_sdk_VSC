...
void Transceiver_loop(struct Transceiver trans){
    if( trans.Name == "serial" ){
        ...
        {
            p = (rec_data_t *)data;
            trans.transceiver_data_proc((char*)(p->data), p->data_length);
...
static char *trans_result_string[CMD_RESULT_CODE_MAX] = 
{
    "\r\nOK\r\n",         //CMD_RESULT_CODE_OK         = 0x00,
    "\r\nERROR(-1)\r\n",  //CMD_RESULT_CODE_ERROR      = 0x01,
    "\r\nERROR(-2)\r\n",
};

void Transceiver_res_result(Transceiver trans, unsigned char result_code)
{
    if (result_code < CMD_RESULT_CODE_MAX) 
    {
        trans.trans_print(trans, trans_result_string[result_code]);
    }
