...
u8 opcode_map[] = OPCODE_MAP;
u32 pin_func_map[] = PIN_FUNC_MAPS;
u32 pin_opt_map[] = PIN_OPT_MAPS;

typedef struct func_pin{
	u32 pin;
}func;

typedef struct opt_pin{
	u32 pin;
}opt;

typedef struct swt{
	func func_pins[PIN_FUNC_NUM];
	opt opt_pins[PIN_OPT_NUM];
	u32 pin_last_pressed;
	char flag_normal_pressed;
	char flag_long_pressed;
	SubAdr SentAdr;
}swt;

swt SWT;

void swt_btn_gpio_init(swt *s){
	for(int i = 0; i< PIN_FUNC_NUM; i++){
		gpio_set_func(s->func_pins[i].pin, AS_GPIO);
		gpio_set_output_en(s->func_pins[i].pin, 0);
		gpio_set_input_en(s->func_pins[i].pin, 1); ;
	}
	for(int i = 0; i< PIN_OPT_NUM; i++){
		gpio_set_func(s->opt_pins[i].pin, AS_GPIO);
		gpio_set_output_en(s->opt_pins[i].pin, 0);
		gpio_set_input_en(s->opt_pins[i].pin, 1); ;
	}
	return;
}

u32 swt_scan_key(){
	char t_pc2 = gpio_read(GPIO_PC2);
	char t_pc3 = gpio_read(GPIO_PC3);
	char t_pc4 = gpio_read(GPIO_PC4);
	if(t_pc2){
		return GPIO_PC2;
	}
	if(t_pc3){
...
void swt_init(swt *s){
	s->flag_long_pressed = 0;
	s->flag_normal_pressed = 0;
	s->pin_last_pressed = 0;
	for(int i = 0; i< PIN_FUNC_NUM; i++){
		s->func_pins[i].pin = pin_func_map[i];
	}
	for(int i = 0; i< PIN_OPT_NUM; i++){
		s->opt_pins[i].pin = pin_opt_map[i];
	}
	swt_btn_gpio_init(s);
	return;
}
...
void swt_test(swt *s){
	static u32 tick_sw_pressed;
	u32 det_sw = swt_scan_key();
	if(det_sw && (s->pin_last_pressed == 0)){
		s->pin_last_pressed = det_sw;
		tick_sw_pressed = clock_time();
	}
	if(det_sw && s->pin_last_pressed){
		if(det_sw == s->pin_last_pressed){
			if (clock_time_exceed(tick_sw_pressed, 2000000)){
				s->flag_long_pressed = 1;
			}
			if(clock_time_exceed(tick_sw_pressed, 200000)){
				s->flag_normal_pressed = 1;
			}
		}else{
			return;
		}
	...
	if(det_sw == 0){
		if(s->flag_long_pressed){
			uart_print_data("1", 1, 0, 0);
			s->flag_long_pressed = 0;
			return; 
		}
		if(s->flag_normal_pressed){
			uart_print_data("1", 1, 0, 0);
			s->flag_normal_pressed = 0;
			return;
		}
...
void switch_set_onoff_to_SentAdr(swt *s, char onoff){
	
		for(int i = 0; i< s->SentAdr.num; i++){
			if(s->SentAdr.subAdr[i] == ADR_ALL_NODES){
				break;
			}
	    	mesh_send_onoff_cmd(s->SentAdr.subAdr[i], onoff);
		}
...
void proc_ui()
{
	static u32 tick_sw_scan, scan_io_interval_us = 4000;
	if (!clock_time_exceed (tick_sw_scan, scan_io_interval_us))
	{
		return;
	}
	tick_sw_scan = clock_time();
